...
function lc_cf7_honeypot_form_tag_handler( $tag ) {
	...
	$html = sprintf(
		...
		esc_html( ! empty( $atts['message'] ) ? $atts['message'] : __( 'If you are human, leave this field blank.', 'lc-mindspace2025' ) ),
		$atts,
		$validation_error
	);
...
function lc_cf7_honeypot_validation_filter( $result, $tag ) {
	...
	if ( ! empty( $value ) ) {
		$result->invalidate( $tag, __( 'Spam detected.', 'lc-mindspace2025' ) );
	}
