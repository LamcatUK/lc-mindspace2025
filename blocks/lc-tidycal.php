<?php // phpcs:ignore 
/**
 * Block template for LC Tidycal.
 *
 * @package lc-mindspace2025
 */

defined( 'ABSPATH' ) || exit;

$class    = $block['className'] ?? '';
$bgcolour = get_field( 'background' ) ? get_field( 'background' ) : 'ivory';
$colour   = get_field( 'cta_colour' ) ? get_field( 'cta_colour' ) : 'white';
$titlecol = 'text-purple-400';

switch ( $colour ) {
    case 'white':
        $content  = '';
        $titlecol = 'text-purple-400';
        break;
    case 'teal-400':
        $content  = 'text-white';
        $titlecol = 'text-white';
        break;
    case 'purple-400':
        $content  = 'text-purple-400';
        $titlecol = 'text-white';
        break;
    default:
        $content  = '';
        $titlecol = 'text-purple-400';
        break;
}

$uq = random_str( 4 );
?>
<div class="buffer-top bg-<?= esc_attr( $bgcolour ); ?>"></div>
<section
    class="cta pt-3 pb-5 bg-<?= esc_attr( $bgcolour ); ?> <?= esc_attr( $class ); ?>">
    <div class="container-xl">
        <div class="cta__card text-center shadow-1 bg-<?= esc_attr( $colour ); ?>">
            <div class="cta__overlay"></div>
            <h2 class="mb-3">
                <?= esc_html( get_field( 'title' ) ); ?>
            </h2>
			<?php
            if ( get_field( 'before' ) ) {
                ?>
            <div class="py-2">
                <?= wp_kses_post( get_field( 'before' ) ); ?>
            </div>
                <?php
            }
            ?>

            <div class="fs-500 mb-2 <?= esc_attr( $titlecol ); ?>"><i class="fa-solid fa-phone"></i>
                <?= wp_kses_post( contact_phone() ); ?>
            </div>
            <div class="fs-500 mb-3"><i class="fa-solid fa-paper-plane"></i>
                <?= wp_kses_post( contact_email() ); ?>
            </div>
			<span class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#bookingCollapse_<?= esc_attr( $uq ); ?>" aria-expanded="false" aria-controls="bookingCollapse_<?= esc_attr( $uq ); ?>">
        		Book a session
    		</span>
            <?php
            if ( get_field( 'after' ) ) {
                ?>
            <div class="has-small-font-size pt-4">
                <?= wp_kses_post( get_field( 'after' ) ); ?>
            </div>
                <?php
            }
            ?>
        </div>
		<div class="collapse" id="bookingCollapse_<?= esc_attr( $uq ); ?>">
        	<div class="tidycal-embed" data-path="uzma1"></div>
		</div>
    </div>
</section>
<div class="buffer-bottom bg-<?= esc_attr( $bgcolour ); ?>"></div>
<?php
// Enqueue the TidyCal script.
wp_enqueue_script(
    'tidycal-embed',
    'https://asset-tidycal.b-cdn.net/js/embed.js',
    array(), // no dependencies.
    null,  // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
    array(
        'strategy'  => 'async',
        'in_footer' => true,
    )
);