...
// Get background color from block settings (theme.json).
$background_color = $block['backgroundColor'] ?? 'white';
$title_col        = 'text-purple-400';
$shadow           = 'shadow-1';

$class = $block['className'] ?? 'py-5';

// Determine text and title colors based on background.
switch ( $background_color ) {
    ...
    default:
        $content   = '';
        $title_col = 'has-purple-400-color';
        break;
}
...
<section
    class="text_image <?= esc_attr( $shadow ); ?> <?= esc_attr( $class ); ?> has-<?= esc_attr( $background_color ); ?>-background-color has-background">
    <?php
    ...
    <div class="container-xl">
        ...
        if ( get_field( 'title' ) ?? null ) {
            ?>
        <div class="h2 text-center d-lg-none mb-4 <?= esc_attr( $title_col ); ?> has-text-color">
        ...
        <div class="row">
            <div
                ...
                ?>
                <div class="<?= esc_attr( $content ); ?> has-text-color"><?= wp_kses_post( get_field( 'content' ) ); ?></div>
